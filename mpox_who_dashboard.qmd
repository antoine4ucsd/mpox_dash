---
title: "WHO mpox clinical data platform"
format: 
  dashboard:
    scrolling: true
    nav-buttons:
...
#| label: load-data
#| message: false
logo<- whomapper::pull_who_logo(region="HQ")
ggsave(here("images/my_logo.png"), plot = grid.raster(logo), width = 4, height = 1, dpi = 300, bg = "white") # 
# htmltools::img(src = knitr::image_uri("my_logo.png"),
#                alt = 'logo',
#                style = 'position:relative; top:0; right:25px; padding:10px; width:25%')
...
# data_redcap<- data_redcap|>type_convert()
...
data_sex_age<- data_redcap|>sex_age_fun()%>%
  ...
  # dplyr::filter(month_start != max(month_start)) %>%
